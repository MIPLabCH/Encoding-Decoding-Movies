...
def load_durations(video, subject):
    ...
    durations_path = '/media/miplab-nas2/Data2/Michael/ml-students2023/resources/run_onsets.pkl'
    onset = pd.read_pickle(durations_path)[video][subject][0]
    video_duration = load_processed_videos(video).shape[0] #TO REPLACE BY THE VALUE IN THE .PKL FILE
...
def split_for_encoder(dataset_ID, test_size = 0.2, mask_size = 4330):
    """
    ...
    fMRIs, videos = load_data()
    video_titles = videos.keys()
    ...
    for key in video_titles:
        ...
        if key == 'YouAgain': #full movie to keep in the test set
