...
from visualisation import plot_train_losses, plot_metrics, plot_decoder_predictions, one_sample_permutation_test
...
def test_model(inputs_dict, labels_dict, model, criterion, device, pretrained_decoder=None, model_to_test=None, statistical_testing = False):
    """
    ...
    for key in inputs_dict.keys():
        ...
        test_loader = torch.utils.data.DataLoader(
            test_set,
            batch_size=16,  # Processing one TR at a time
            shuffle=False,
            pin_memory=torch.cuda.is_available(),
            num_workers=4
        )
        ...
        if model_to_test != 'decoder':
            all_encoded = results['encoder_predictions'][key]
            all_labels = labels_dict[key]
            plot_metrics(all_labels, all_encoded, key, False)

    if model_to_test != 'decoder':
        all_encoded, all_labels = [], []
        for key in results['encoder_predictions'].keys():
            all_encoded.append(results['encoder_predictions'][key])
            all_labels.append(labels_dict[key])
        all_encoded = np.concatenate(all_encoded, axis=0)
        all_labels = np.concatenate(all_labels, axis=0)
        plot_metrics(all_labels, all_encoded, 'all', True)
        if statistical_testing:
            one_sample_permutation_test(all_labels, all_encoded)

    if model_to_test != 'encoder' or pretrained_decoder is not None:
        if model_to_test == 'decoder':
            plot_decoder_predictions(results['decoder_predictions'], labels_dict)
        else:
            plot_decoder_predictions(results['decoder_predictions'], inputs_dict)
