...
#mask_size = 4936 # number of voxels in the preprocessed fMRI data. either 4609 or 15364
mask_size = 4609
#trainset0, valset0, testset0 = get_dataset(dataset_ID, mask_size) # data are loaded into dictionaries


#load dataset with FFA from subject 1
...
trainset, valset, testset = get_dataset4()
valset=testset
...
save_model_as = '/media/RCPNAS/MIP/Michael/students_work/rodrigo/temporal_decoder_' + str(mask_size) + '_' + str(num_epochs) + '_TRwindow' + str(window_size) + '_l1_' + str(lambda_l1) + '_ffaS01_datafrom_old_processed_videosnpload'
...
display_plots = True
save_plots = True
model_name = 'temporal_decoder_' + str(mask_size) + '_' + str(num_epochs) + '_TRwindow' + str(window_size) + '_l1_' + str(lambda_l1) + '_ffaS01_datafrom_old_processed_videosnpload'
...
model, history = train_model_temporal_with_val_and_reg(
    ...
    batch_size, device, save_model_as, pretrained_decoder, 
    start_epoch, start_loss, model_to_train, 
    display_plots, save_plots, model_name, lambda_l1=lambda_l1
)
